...
const CampridgeProject = ({ language }) => {
  ...
  const featureContent = language === "English" ? (
    <div>
      ...
      <p>
        ...
        various locations (campsites, beaches, fishing spots, rest areas, etc.).
        <br />
      ...
      <p>
        ...
        <br />
        &nbsp;&nbsp;* For search, send the query to SearchController → integrated
        search → display results in the app.
        <br />
        &nbsp;&nbsp;* When a marker is clicked, WebView → sends a message to RN →
        navigation.navigate(...) to enter the detail page.
      </p>
      <br />
      <p>
        ...
        stores it in the DB.
        <br />
        &nbsp;&nbsp;* For traffic control, updates periodically via a scheduler.
        <br />
        &nbsp;&nbsp;* The app communicates with the server using AxiosInstance,
        requests data with useEffect / useFocusEffect, and manages state.
      </p>
      <br />
      <p>
      ...
      <p>
        * Kakao/Google Login
        ...
        <br />
        &nbsp;&nbsp;* After logging in, receives Access Token & Refresh Token and
        stores them in AuthContext.
      </p>
      <br />
      <p>
        * Email Sign-up
      ...
      <p>
        * AuthContext
        ...
        <br />
        &nbsp;&nbsp;* On screen transitions (checking userId validity) → if user
        already wrote a review, the button is disabled, etc.
      </p>
...
`}
      ...
      <p>
        * MapView.js:
        ...
        'mapReady' → RN sends initial data (marker list, favorites list, etc.).
        <br />
...
`}
      ...
      <p>
        * Search
        <br />
        &nbsp;&nbsp;* Sends query to SearchController → integrated search across
        multiple Repositories for campsites, fishing spots, etc. → returns DTO.
        <br />
        &nbsp;&nbsp;* RN displays the search results as markers on the map or as
        a list.
      </p>
      <br />
      <p>
      ...
      <p>
        ...
        * Nickname update: PATCH/PUT request to the server.
        <br />
        * Boot Splash: loads the app for the first time (data load + map
      ...
      <p>
        * SecurityConfig
        ...
        UsernamePasswordAuthenticationFilter, checks token & if expired, checks
        Refresh Token.
      </p>
      <p>
        * JwtTokenUtil
        ...
        <br />
        &nbsp;&nbsp;* If the token is expired, refreshAccessToken(refreshToken)
        to renew.
      </p>
      <pre style={{ background: "#f6f6f6", padding: "8px" }}>
        {`java
...
`}
      ...
      <p>
        * ReviewController
        ...
        <br />
        &nbsp;&nbsp;* Average rating:
        /api/reviews/average/{`{contentType}`}/{`{contentId}`}
      </p>
      <p>
        * SearchController
        <br />
        &nbsp;&nbsp;* /api/search?query=... → returns integrated search results
        for campsites, fishing spots, etc.
      </p>
      <br />
      <p>
      ...
      <p>
        * Entity
        ...
        <br />
        &nbsp;&nbsp;* Review: user, contentType, contentId, rating, content, etc.
        <br />
      ...
      <p>
        * Encoding: set to utf8mb4 to avoid broken Korean characters in AWS RDS.
        <br />* Scheduler: refreshes DB periodically to prevent API traffic issues.
      </p>
      <pre style={{ background: "#f6f6f6", padding: "8px" }}>
        {`java
// JwtTokenUtil.java
...
`}
      ...
      <p>
        ...
        <br />
        3. Enters the map screen (WebView) → loads place data (campsites, etc.)
        from the server → displays Leaflet markers.
        <br />
        4. User location updates (Geolocation) → sends updateLocation message to
        WebView → moves user marker.
        <br />
        5. Click on Favorites button → toggles favoriteMarkers layer on Leaflet.
        <br />
        &nbsp;&nbsp;&nbsp;* On the server side, /favorites/user/:userId is used
        to store/delete from DB.
        <br />
        6. Click a marker (e.g., a certain campsite) → popup → on tap, type:
        'campgroundSelected' → RN navigates to detail screen (photos, reviews,
        rating, etc.).
        <br />
        7. If the token is expired when writing a review, automatically use the
        Refresh Token to reissue a new token.
        <br />
    ...
    <div>
      ...
      <p>
        ...
        방식을 지원.
        <br />
        &nbsp;&nbsp;* 인증 시 Spring Boot 서버로 요청, JWT(Access, Refresh) 토큰
        발급받음.
      </p>
      <br />
      <p>
        ...
        <br />
        &nbsp;&nbsp;* 앱 구동 시, RNBootSplash로 로딩 화면 → 데이터 로딩 & 지도
        준비 끝나면 스플래시 종료.
      </p>
      <br />
      <p>
        ...
        <br />
        &nbsp;&nbsp;* Leaflet + OpenStreetMap 기반, 각종 장소(캠핑장, 해수욕장,
        낚시터, 휴게소 등) 마커 표시.
        <br />
      ...
      <p>
        ...
        <br />
        &nbsp;&nbsp;* 검색 시 SearchController로 쿼리 전송 → 통합 검색 → 결과를
        앱에 표시.
        <br />
      ...
      <p>
        5) <b>데이터 흐름 & 스케줄러</b>
        <br />
        &nbsp;&nbsp;* 공공데이터 API + 클라이언트 JSON에서 초기 데이터를 불러와
        DB 저장.
        <br />
...
`}
      ...
      <p>
        * MapView.js:
        ...
        <br />
        &nbsp;&nbsp;* 마커 클릭 시 JS 쪽에서 type: '...Selected' 메시지 → RN에서
        상세 화면 이동.
      </p>
      <pre style={{ background: "#f6f6f6", padding: "8px" }}>
        {`jsx
...
`}
      ...
      <p>
        * 즐겨찾기
        <br />
        &nbsp;&nbsp;* Favorites 버튼 토글 시, RN에서 showFavorites = true 메시지
        전송 → Leaflet에 favoriteMarkers 레이어 add/remove.
        <br />
...
`}
      ...
      <p>
        * AuthController
        ...
        <br />
        &nbsp;&nbsp;* /api/auth/kakao, /api/auth/google → OAuth 토큰 검증 & 유저
        정보 연동.
      </p>
      <p>
        * ReviewController
        <br />
        &nbsp;&nbsp;* /api/reviews/{`{contentType}`}/{`{contentId}`} → 해당 컨텐츠
        리뷰 목록 조회.
        <br />
      ...
      <p>
        * SearchController
        <br />
        &nbsp;&nbsp;* /api/search?query=... → 캠핑장, 낚시터 등 공통 엔티티 검색
        결과 반환.
      </p>
      <br />
      <p>
...
`}
      ...
      <p>
        ...
        <br />
        7. 리뷰 작성 시 토큰 만료되었다면 Refresh Token 자동 사용, 새 토큰 재발급.
        <br />
      ...
      <p>
        * Back-end(Spring Boot):
        ...
        <br />
        &nbsp;&nbsp;* DB(MySQL/MariaDB) + utf8mb4 인코딩, 스케줄러로 공공데이터
        API 갱신.
        <br />
  ...
  const troubleshootContent = language === "English" ? (
    <div>
      ...
      <p>
        ...
        <br />
        * Issue: Even though there was already a dedicated layer for favorites,
        it was handled like a normal marker → duplication/error
        <br />
        * Cause: The favoriteMarkers layer must be managed independently by
        map.addLayer() / map.removeLayer(), but it was mixed with normal markers
        <br />
        * Solution:
        <br />
      ...
      <p>
        ...
        * Solution:
        <br />
        &nbsp;&nbsp;&nbsp;* Added userId to JWT claims when generating the token,
        <br />
        &nbsp;&nbsp;&nbsp;* Managed userId in AuthContext,
        <br />
        &nbsp;&nbsp;&nbsp;* Called checkAuthStatus() on screen focus to
        reconfirm review existence
        <br />* Result: The review remains, and the button is disabled for a user
        who has already written a review
      </p>
      <br />
      <p>
        <b>5) KakaoNavi Coordinate Transfer Issue</b>
        <br />
        * Issue: Unable to directly pass coordinates to KakaoNavi(Android Intent)
    ...
    <div>
      ...
      <p>
        ...
        * 이슈: 공공 API 호출 시 키가 중복 인코딩되어 401 오류 발생
        <br />
        * 원인: 이미 인코딩된 서비스 키를 RestTemplate + UriComponentsBuilder가 또
        인코딩
        <br />
        * 해결:
        <br />
        ...
        <br />
        &nbsp;&nbsp;&nbsp;* UriComponentsBuilder.build(false)로 추가 인코딩 방지,
        <br />
        &nbsp;&nbsp;&nbsp;* 헤더/파싱 로직 재정비
      ...
      <p>
        ...
        * 이슈: 이미 즐겨찾기 전용 레이어가 있음에도, 일반 마커 로직처럼 처리 →
        중복/오류
        <br />
        * 원인: favoriteMarkers 레이어를 map.addLayer() / map.removeLayer()로 독립
        관리해야 하는데, 일반 마커와 혼용
        <br />
        * 해결:
        <br />
      ...
      <p>
        ...
        <br />
        * 이슈: RN 환경에서 카카오내비(안드로이드 Intent)로 좌표 직접 전달 불가
        <br />
  ...
  const flowchartContent = language === "English" ? (
    <div>
      <h3>Flowchart</h3>
  ...
  // (이미지는 그대로, 탭 제목 및 부연 텍스트만 번역)
  const erdContent = language === "English" ? (
    <div>
      <h3>ERD Diagram</h3>
        ...
        >
          ...
          <ModalContent>
            ...
            <ToggleButtonWrapper>
              ...
              >
                {language === "English" ? "ERD Diagram" : "ERD그램"}
              </ToggleButton>
            </ToggleButtonWrapper>
